...
public:
	...
	void time_step()
	{
		...
		Yv = 0.5*(-40*1000*abs(nu_r(1)))*(1.1+0.0045*(1.01/0.09)-0.1*(0.27/0.09)+0.016*(pow((0.27/0.09),2)))*(3.141592*0.09*1.01/2.0);
		Yr = 3*(-3.141592*1000)*sqrt(pow(nu_r(0),2)+pow(nu_r(1),2))*0.09*0.09*1.01;
		...
		CA << 0.00, 0.00, 2 * ((Y_v_dot*nu_r(1)) + ((Y_r_dot + N_v_dot)/2) * nu_r(2)),
			0.00, 0.00, 0.00 - X_u_dot * nu_r(0),
			2*(((0.00 - Y_v_dot) * nu_r(1)) - ((Y_r_dot+N_v_dot)/2) * nu_r(2)), X_u_dot * nu_r(0), 0.00;
...
int main(int argc, char *argv[])
{
	...
	int rate = 100;
	dynamicModel.integral_step = 1.0/100.0;//0.001;
